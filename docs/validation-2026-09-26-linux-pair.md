...
The two NICs were cabled directly with one NVIDIA-coded 200G QSFP112 passive copper cable; the link negotiated 100 Gb/s (ethtool on host B: `Speed: 100000Mb/s`). Both ports carried IPv4 addresses on one private /24. No switch.
...
48 benchmark processes (16 configurations, 3 measured trials each, plus warmups), 0 failed, 0 completion errors, 0 mismatches, `guard_ok=1` on every row. Medians of three, Gbit/s (`benchmarks/bw_summary.py` over the retained CSVs):
...
By payload direction: host B to host A (A READ, B WRITE) ran at 30.1 to 30.5 Gbit/s; host A to host B (A WRITE, B READ) at 29.3 to 29.8. Every configuration sits within 1.3 Gbit/s of 30, independent of request size and depth. An earlier run the same day, with the endpoint handshake relayed by hand instead of by the runner (RoCE v1, GID index 0, 1 MiB, depth 7, A WRITE), measured 29.75, 29.72 and 29.75 Gbit/s, in line with the table.

Verification is sampled, as in the other reports: the verified column is the number of bytes the receiver checked against the seeded pattern per trial, not the payload size.

## Observations

- **A flat ~30 Gbit/s in both directions.** The link negotiated 100 Gb/s and host B's ConnectX-7 is internal, so the ceiling is on host A's side. The Helios enclosure reaches host A over USB4, and a PCIe tunnel on that path is the likely limit. We did not measure the tunnel separately, so this is an attribution, not a measurement. For comparison, the 17 September report above measured the same Helios and card on a Mac Studio over Thunderbolt 5 at about 50.5 Gbit/s into the Mac and 29.4 out; here the into-host direction is also held at about 30.
...
- **RoCE v2 over link-local GIDs did not connect on this pair.** A separate perftest check with the MAC-derived link-local RoCE v2 GIDs failed at `modify QP to RTR`; the IPv4-mapped RoCE v2 GIDs above connected at once. The runner accepts IPv4-mapped GIDs only with `--mac-platform linux` and checks the IPv4 neighbour table for them; the macOS path still requires the MAC-derived link-local GID and static IPv6 neighbours.
- **Host B's netdev MTU changed from 1500 to 9000 during the sweep.** The RDMA path MTU was fixed at 1024 by `--mtu` throughout, and no step is visible in the rows.
